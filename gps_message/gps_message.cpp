...
void GpsMessage::data_filter(char *buf, char *str)
{
	int count = 0;
	int j = 0;
	int i = 0;
...
int GpsMessage::gprmc_analysis(gps_info *p, char *str)
{
	...
	GpsMessage::data_filter( * buf, str);
	// printf(" \n ");
	// for (i = 0; i < 12; i++)
	// {
	// 	printf("buf[%d]= %s \n ",i, buf[i]);
...
* FAILURE -1.
*/
int GpsMessage::gpgga_analysis(gps_info *p, char *str)
{
	char buf[32][128];
	int i;
	if (str[4] != 'G')
...
char * GpsMessage::assemble(gps_info *p,char * assembled)
{
	gps_info  gps_st;
	gps_st = *p;
	char  temp[30] ;
	char line[] = "*************************************************\n";
	strcat(assembled,line);//拼接字符串
	sprintf( temp, "%c %c\n" ,gps_st.ns, gps_st.ew );

	strcat(assembled,temp);//拼接字符串
	sprintf(temp,"latitude:%d度%d分%d秒\n", gps_st.latitude_degree, gps_st.latitude_cent, gps_st.latitude_second);
	strcat(assembled,temp);//拼接字符串
	sprintf(temp,"longitude:%d度%d分%d秒\n", gps_st.longitude_degree, gps_st.longitude_cent, gps_st.longitude_second);
	strcat(assembled,temp);//拼接字符串 
	sprintf(temp,"speed:%10lf    direction:%10lf\n", gps_st.speed, gps_st.direction);
	strcat(assembled,temp);//拼接字符串
	sprintf(temp,"%d年%d月%d日%d时%d分%d秒\n", gps_st.time.year, gps_st.time.month, gps_st.time.day, gps_st.time.hour, gps_st.time.minute, gps_st.time.second);
	strcat(assembled,temp);//拼接字符串
	sprintf(temp,"海拔高度 %lf, 卫星编号 %d", gps_st.height, gps_st.satellite);
	strcat(assembled,temp);//拼接字符串
	// printf("assembled c %c",assembled);
	// printf("%c %c\n", gps_st.ns, gps_st.ew);
	// printf("%d度%d分%d秒\n", gps_st.latitude_degree, gps_st.latitude_cent, gps_st.latitude_second);
	// printf("%d度%d分%d秒\n", gps_st.longitude_degree, gps_st.longitude_cent, gps_st.longitude_second);
	// printf("%lf %lf\n", gps_st.speed, gps_st.direction);
...
int main()
{
	...
	GM.gprmc_analysis(&gps_st, tmp);
	GM.gpgga_analysis(&gps_st, tmp2);
	// char * ss = GM.assemble(&gps_st,log);
	printf("%s",GM.assemble(&gps_st,log));
	return 0;
}
